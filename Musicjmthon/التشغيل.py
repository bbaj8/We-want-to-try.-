    ...
    if replied:
        if replied.audio or replied.voice:
            ...
            if chat_id in QUEUE:
                ...
                await m.reply_photo(
                    photo="https://telegra.ph/file/2e08e1a08858d2ea98a40.jpg",
                    caption=f"""
**العنوان حبي : [{songname}]({link})
 ايدي : {chat_id}
من طلب هذا الكواد : {m.from_user.mention}**
""",
                )
            else:
...
""",
        ...
        if len(m.command) < 2:
            ...
            await m.delete()
            huehue = await m.reply("يبن القواد جاري البحث ")
                ...
                if hm == 0:
                    ...
                    if chat_id in QUEUE:
                        ...
                        await m.reply_photo(
                            photo=f"{thumbnail}",
                            caption=f"""
**العنوان حبي : [{songname}]({url})
وقت طيويو : {duration}
 ايدي  : {chat_id}
هذا الكواد راد الاغنيه : {m.from_user.mention}**
""",
                        )
                    else:
                        try:
...
**▶ بدأ تشغيل اقاني
**العنوان حبي : [{songname}]({url})
مده الطيوطو : {duration}
 ايدي  : {chat_id}
من طلب هذا : {m.from_user.mention}**
""",
                            )
